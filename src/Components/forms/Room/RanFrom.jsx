...
const RanFrom = () => {
  ...
  const [technologyOptions, setTechnologyOptions] = useState(['2G', '3G', '4G', '5G', 'Other']);
  ...
  const saveDataToAPI = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // New fields from image
        ran_equipment_vendor: formData.ran_equipment_vendor || [],
        free_slots_in_ran_racks: formData.free_slots_in_ran_racks || '',
        how_many_racks_for_ran_bts: formData.how_many_racks_for_ran_bts || '',
        rack_type_with_free_slots: formData.rack_type_with_free_slots || [],
        available_locations_for_nokia_hw: formData.available_locations_for_nokia_hw || [],
        transmission_cable_length: parseFloat(formData.transmission_cable_length) || 0
      };
      ...
      await axios.put(`${import.meta.env.VITE_API_URL}/api/ran-room/${sessionId}`, submitFormData, {
        headers: {
  ...
  const getImageFields = () => {
    const fields = [];
    const btsCount = parseInt(formData.how_many_racks_for_ran_bts) || 0;

    // Add RAN equipment photo
    fields.push(
      { label: `RAN equipment photo`, name: `ran_equipment_photo` }
    );

    // Add dynamic BTS photo fields based on count
    for (let i = 1; i <= btsCount; i++) {
      fields.push(
        { label: `Rack Photo ${i}`, name: `rack_photo_${i}` },
        { label: `Rack photos 1. closed ${i}`, name: `rack_photo_1_closed_${i}` },
        { label: `Rack photos 2. inside ${i}`, name: `rack_photo_2_inside_${i}` },
        { label: `Rack photos 3. zoomed on upper part ${i}`, name: `rack_photo_3_zoomed_on_upper_part_${i}` },
        { label: `Rack photos 4. zoomed on lower part ${i}`, name: `rack_photo_4_zoomed_on_lower_part_${i}` },
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/api/ran-room/${sessionId}`)
      .then(res => {
        const data = res.data.data || res.data;
        console.log("Fetched RAN room data:", data);
        ...
        if (data) {
          ...
          setFormData({
            ...
            // New fields from image
            ran_equipment_vendor: ranData.ran_equipment_vendor || [],
            free_slots_in_ran_racks: ranData.free_slots_in_ran_racks || '',
            how_many_racks_for_ran_bts: ranData.how_many_racks_for_ran_bts || '',
            rack_type_with_free_slots: ranData.rack_type_with_free_slots || [],
            available_locations_for_nokia_hw: ranData.available_locations_for_nokia_hw || [],
            transmission_cable_length: ranData.transmission_cable_length || ''
          });
      ...
      .catch(err => {
        console.error("Error loading RAN room data:", err);
  ...
  const fetchTechnologyOptions = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/ran-room/${sessionId}/technology-options`);
  ...
  const refreshCabinetCount = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/ran-room/${sessionId}`);
      const data = response.data.data || response.data;
  ...
  const rackTypeOptions = ['Open 19" rack', 'Closed 19" rack', 'Other'];
  const availableLocationOptions = ['Wall mount', 'In existing rack', 'New rack', 'Other'];
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="mb-8  p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 bg-gray-100 p-4 rounded-lg mt-10">
                ...
                <div>
                  <label className='block font-semibold mb-2'>How many Racks for Ran BTS</label>
                  <input
                    type='number'
                    className='form-input'
                    value={formData.how_many_racks_for_ran_bts}
                    onChange={(e) => handleChange('how_many_racks_for_ran_bts', e.target.value)}
                    placeholder='000'
                  />
                </div>
                {/* Free slots in existing RAN racks */}
                <div>
                  <label className='block font-semibold mb-2'>Is there free slots in existing RAN racks for new Nokia HW installation?</label>
                  <div className='flex gap-4'>
                    {['Yes', 'No'].map((option) => (
                      <label key={option} className="flex items-center gap-2">
                        <input
                          type="radio"
                          name="free_slots_in_ran_racks"
                          value={option}
                          checked={formData.free_slots_in_ran_racks === option}
                          onChange={(e) => handleChange('free_slots_in_ran_racks', e.target.value)}
                          className="w-4 h-4"
                        />
                        {option}
                      </label>
                ...
                {/* Type of rack with free slots */}
                <div>
                  <label className='block font-semibold mb-2'>Type of rack which has free slots</label>
                  <div className='grid grid-cols-3 gap-2'>
                    {rackTypeOptions.map((rackType) => (
                      <label key={rackType} className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          value={rackType}
                          checked={formData.rack_type_with_free_slots.includes(rackType)}
                          onChange={(e) => handleCheckboxChange('rack_type_with_free_slots', e.target.value)}
                          className="w-4 h-4"
                        />
                        {rackType}
                      </label>
                ...
                {/* Available locations for Nokia HW */}
                <div>
                  <label className='block font-semibold mb-2'>Is there more than location available for new Nokia HW? Choose all applicable</label>
                    ...
                    {availableLocationOptions.map((location) => (
                      <label key={location} className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          value={location}
                          checked={formData.available_locations_for_nokia_hw.includes(location)}
                          onChange={(e) => handleCheckboxChange('available_locations_for_nokia_hw', e.target.value)}
                          className="w-4 h-4"
                        />
                        {location}
                      </label>
