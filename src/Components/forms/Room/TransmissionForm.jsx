...
const TransmissionMWForm = () => {
  ...
  const saveDataToAPI = async () => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/transmission-room/${sessionId}`,
        submitFormData,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/api/transmission-room/${sessionId}`)
      .then(res => {
  ...
  const handleChange = (name, value) => {
    ...
    setFormData((prev) => {
      ...
      if (name === "space_available") {
        const current = prev.space_available || [];
        // toggle
        if (current.includes(value)) {
          newFormData.space_available = current.filter((v) => v !== value);
        } else {
          newFormData.space_available = [...current, value];
        }
      } 
      
      else if (name === "how_many_mw_link_exist") {
        ...
        newFormData.how_many_mw_link_exist = value;
      } 
  ...
  const mwVendors = ['Nokia', 'Ericsson', 'Huawei', 'ZTE', 'Other'];
        ...
        <form className="flex-1 flex flex-col min-h-0" onSubmit={handleSubmit}>
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              {/* Is space available for new MW IDU installation? Choose all applicable */}
              <div className=''>
                <label className='block font-semibold mb-2'>Is space available for new MW IDU installation? Choose all applicable</label>
                <div className='space-x-4'>
                  {['Wall mount', 'In existing rack', 'New rack', 'Other'].map((option) => (
                    <label key={option} className='inline-flex items-center'>
                      <input
                        type="checkbox"
                        name="space_available"
                        value={option}
                        checked={formData.space_available.includes(option)}
                        onChange={(e) => handleChange('space_available', e.target.value)}
                      />
